// Updated getActivities function for lead.controller.js
...
const getActivities = asyncHandler(async (req, res, next) => {
  try {
    ...
    // Get limit from query or use default (allow higher limits for the modal view)
    ...
    const enrichedActivities = activities.map(activity => {
      ...
      // We keep both original fields (user, lead) and expected fields (userInfo, leadInfo)
      // to support both data structures until frontend is fully migrated
      return {
        ...activity,
        timeAgo,
        activityLabel,
        ...
        formattedDuration,
        formattedDate,
        userInfo: activity.user,  // For backward compatibility
        leadInfo: activity.lead   // For backward compatibility
      };
    });
